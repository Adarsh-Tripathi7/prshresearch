...
window.loadTimeframeData = function(tf) {
  const loader = $('loader');
  ...
  window.onDataLoaded = function() {
    try {
      ...
      console.error("Failed to process data for", tf, e);
    } finally {
  ...
  script.src = `data/${tf}.js`;
  script.onerror = function() {
      console.error("Failed to load data for", tf);
...
function showSpotlight(win) {
  ...
  if (showProb && p) {
    let innerCombined = '';
    if (assets.includes('nq')) innerCombined += `<div class="spot-row"><span class="lbl">NQ</span><span class="val c-dn">${p.BNF_DB_Prob.toFixed(1)}%</span></div>`;
    if (assets.includes('es')) innerCombined += `<div class="spot-row"><span class="lbl">ES</span><span class="val c-dn">${p.BNF_DB_Prob.toFixed(1)}%</span></div>`;
    ...
    if (currentView === 'page-probability' && pf) {
        let innerHigh = '';
        if (assets.includes('nq')) innerHigh += `<div class="spot-row"><span class="lbl">NQ</span><span class="val c-dn">${pf.BNF_H_First_DB_Prob?.toFixed(1) || '—'}%</span></div>`;
        if (assets.includes('es')) innerHigh += `<div class="spot-row"><span class="lbl">ES</span><span class="val c-dn">${pf.BNF_H_First_DB_Prob?.toFixed(1) || '—'}%</span></div>`;
        html += `<div class="spot-card"><div class="spot-card-title">Double Break % (High First)</div>${innerHigh}</div>`;
        
        let innerLow = '';
        if (assets.includes('nq')) innerLow += `<div class="spot-row"><span class="lbl">NQ</span><span class="val c-dn">${pf.BNF_L_First_DB_Prob?.toFixed(1) || '—'}%</span></div>`;
        if (assets.includes('es')) innerLow += `<div class="spot-row"><span class="lbl">ES</span><span class="val c-dn">${pf.BNF_L_First_DB_Prob?.toFixed(1) || '—'}%</span></div>`;
...
// Global UI State
let globalAsset = 'bnf'; // 'both', 'nq', 'es'
...
function updateKPIs() {
  ...
  probData.forEach(d => {
    let a;
    if (globalAsset === 'both') a = (d.BNF_DB_Prob + d.BNF_DB_Prob) / 2;
    else if (globalAsset === 'nq') a = d.BNF_DB_Prob;
    else a = d.BNF_DB_Prob;
...
function updateOverviewCharts() {
  ...
  const nq = probData.map(d => d.BNF_DB_Prob);
  const es = probData.map(d => d.BNF_DB_Prob);
  
  const seriesProb = [{ name: 'BNF', type: 'line', data: nq, smooth: 0.3, symbolSize: 6, itemStyle: { color: '#6366f1' }, lineStyle: { width: 3 }, areaStyle: { color: new echarts.graphic.LinearGradient(0,0,0,1, [{offset:0, color:'rgba(99,102,241,0.2)'},{offset:1, color:'rgba(99,102,241,0)'}]) } }];
  ...
  const seriesExt = [{ name: 'BNF', type: 'bar', data: nqExt, itemStyle: { color: '#10b981', borderRadius: [4,4,0,0] }, barMaxWidth: 16 }];
...
function updateProb() {
  ...
  let mappedData = data.map(d => {
    const n = d.BNF_DB_Prob;
    const e = d.BNF_DB_Prob;
    return { Window: d.Window, nq: n, es: e, avg: n, delta: 0 };
...
function updateHlFirst() {
  ...
  let mappedData = data.map(d => {
    ...
    if (dir === 'high_first') {
        ...
        e = d.BNF_H_First_L_Break_Prob;
        n_cond = d.BNF_H_First_CBM_Prob;
        e_cond = d.BNF_H_First_CBM_Prob;
    } else if (dir === 'low_first') {
        n = d.BNF_L_First_H_Break_Prob;
        e = d.BNF_L_First_H_Break_Prob;
        n_cond = d.BNF_L_First_CAM_Prob;
        e_cond = d.BNF_L_First_CAM_Prob;
    } else {
        n = d.BNF_Comb_Opp_Prob;
        e = d.BNF_Comb_Opp_Prob;
        n_cond = d.BNF_Comb_Cond_Prob;
        e_cond = d.BNF_Comb_Cond_Prob;
    }
  ...
  const series = [{ name: 'BNF', type: 'line', data: nqVals, smooth: 0.3, symbolSize: 6, lineStyle: { width: 3, color: '#10b981' }, itemStyle: { color: '#10b981' } }];
  ...
  let th = '<tr>' + thHtml('Window', 'Window', '');
  if (showNq) {
      th += thHtml('nq', 'Opp Break BNF %', 'r');
      th += thHtml('nq_cond', 'Cond Opp Break BNF %', 'r');
  }
  if (showEs) {
...
function updateHeatmap() {
  ...
  if (!c._hasSort) {
    c.on('click', function(p) {
      if (p.componentType === 'xAxis') {
        ...
        const allLabels = [...extLabels, 'BNF Lead +`', 'BNF Lead +"', 'Combined Avg', 'BNF'];
  ...
  if (type === 'prob') {
    ...
    if (hmSortDir !== 0 && hmSortCol) {
       sortedProb.sort((a,b) => {
           let vA = a.BNF_DB_Prob;
           let vB = b.BNF_DB_Prob;
    ...
    if (globalAsset === 'both') {
      ...
      sortedProb.forEach((d, yi) => { 
        const val = globalAsset === 'nq' ? d.BNF_DB_Prob : d.BNF_DB_Prob;
    ...
    const refData = EXT['bnf']?.[btype]?.[dir];
    ...
    if (hmSortDir !== 0 && hmSortCol) {
       ...
       if (p) {
           sortedRef.sort((a, b) => {
               let rNqA = EXT['bnf']?.[btype]?.[dir]?.find(x => x.Window === a.Window);
               let rEsA = EXT['bnf']?.[btype]?.[dir]?.find(x => x.Window === a.Window);
               let vA = 0, vB = 0;
               if (globalAsset === 'both') { vA = ((rNqA?.[p]||0) + (rEsA?.[p]||0))/2; }
               else if (globalAsset === 'nq') { vA = rNqA?.[p]||0; } else { vA = rEsA?.[p]||0; }
               
               let rNqB = EXT['bnf']?.[btype]?.[dir]?.find(x => x.Window === b.Window);
               let rEsB = EXT['bnf']?.[btype]?.[dir]?.find(x => x.Window === b.Window);
