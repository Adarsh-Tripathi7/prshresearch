...
def main():
    print("\n" + "=" * 70)
    print("  NQ 0:46-0:52 RANGE BREAKDOWN SHORT -- TARGET SIZE COMPARISON")
    ...
    for tag, trades in [("target_025", trades_a), ("target_050", trades_b),
        ...
        path = os.path.join(results_dir, f"strategy_0046_short_{tag}.csv")
    ...
    pd.DataFrame([stats_row(s) for s in [sa, sb, sc, sd] if s]).to_csv(
        os.path.join(results_dir, "strategy_0046_target_comparison.csv"), index=False)
    print("\n  Done.\n")
