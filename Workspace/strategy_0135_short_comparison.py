...
def backtest_core(dates_int, times_min, highs, lows, closes,
                  ...
                  invalidate_on_high=True, friction_pts=0.0):
    """
    ...
    for d in range(n_days):
                ...
                if breaks_low and breaks_high:
                    if invalidate_on_high:
                        ...
                        if hits_tp and hits_sl:
                            ...
                            r_mult = pnl / R; exit_px = sl_px; exit_min = m
                            resolved = True; break
                elif breaks_high and invalidate_on_high:
        ...
        if entered and not resolved:
            ...
            pnl = (entry_px - eod_close) - friction_pts
...
def main():
    print("\n" + "=" * 70)
    print("  NQ 1:35-1:51 RANGE BREAKDOWN SHORT -- TARGET SIZE COMPARISON")
    print("  All scenarios: Invalidate if High breaks first | WITH COSTS (0.725 pts)")
    print("=" * 70)
    ...
    for tag, trades in [("target_025", trades_a), ("target_050", trades_b),
        ...
        path = os.path.join(results_dir, f"strategy_0135_short_{tag}.csv")
    ...
    pd.DataFrame([stats_row(s) for s in [sa, sb, sc, sd] if s]).to_csv(
        os.path.join(results_dir, "strategy_0135_target_comparison.csv"), index=False)
    print("\n  Done.\n")
