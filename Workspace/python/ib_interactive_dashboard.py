...
DATA_DIR = r"d:\Antigravity\Results\IB Analysis"
OUT_PATH = os.path.join(DATA_DIR, "ib_interactive_dashboard.html")
...
# Load all datasets
prob = read_csv_json("db_probability_by_rth_close.csv")

ext = {}
for asset in ["nq", "es"]:
    ext[asset] = {}
    for btype in ["double_break", "single_break"]:
        ext[asset][btype] = {}
        for direction in ["combined", "high_first", "low_first"]:
            fname = f"{asset}_{btype}_{direction}_percentiles.csv"
            ext[asset][btype][direction] = read_csv_json(fname)

# Serialize data
data_block = json.dumps({"prob": prob, "ext": ext}, separators=(',',':'))
...
<title>IB Research Terminal — NQ & ES</title>
...
const TT={backgroundColor:'#1e2228',borderColor:'rgba(255,255,255,0.08)',textStyle:{fontFamily:'IBM Plex Mono',fontSize:11,color:'#a0a4ae'},padding:[8,12],extraCssText:'border-radius:6px;box-shadow:0 4px 20px rgba(0,0,0,.4)'};
...
(function(){
  ...
  $('kpiStrip').innerHTML=`
    <div class="metric"><div class="metric-label">Windows Analyzed</div><div class="metric-value" style="color:var(--text-1)">42</div><div class="metric-sub">60m rolling · 30m step</div></div>
...
(function(){
  ...
  ec('chartProbFull').setOption({
    ...
    series:[
      ...
      {name:'50%',type:'line',data:labels.map(()=>50),lineStyle:{width:1,color:'#3e424c',type:'dashed'},symbol:'none',silent:true}
    ]
  });
  // Table
  let h='';
  probData.forEach(d=>{
    const avg=(d.NQ_DB_Prob+d.ES_DB_Prob)/2;
    const delta=d.NQ_DB_Prob-d.ES_DB_Prob;
    const dc=delta>0?'c-dn':'c-up';
    const bg=avg>75?'var(--dn-dim)':avg<35?'var(--up-dim)':'transparent';
    h+=`<tr style="background:${bg}"><td class="c-mono">${d.Window}</td><td class="r c-mono">${d.NQ_DB_Prob.toFixed(1)}</td><td class="r c-mono">${d.ES_DB_Prob.toFixed(1)}</td><td class="r c-mono">${avg.toFixed(1)}</td><td class="r c-mono ${dc}">${delta>0?'+':''}${delta.toFixed(1)}</td></tr>`;
  });
  $('tblProb').innerHTML=h;
})();
...
function updateExt(){
  ...
  const brk=segVal('segBreak')==='double_break'?'Double Break':'Single Break';
  ...
  const lc=segVal('segBreak')==='double_break'?'#f87171':'#34d399';
  const ac=segVal('segBreak')==='double_break'?'rgba(248,113,113,.1)':'rgba(52,211,153,.1)';
  ...
  ec('chartCdf').setOption({
    ...
    series:[{name:brk,type:'line',data:vals,smooth:.3,symbol:'circle',symbolSize:4,lineStyle:{width:2.5,color:lc},itemStyle:{color:lc},
      ...
      markLine:{silent:true,data:[{yAxis:1,lineStyle:{color:'#3e424c',type:'dashed',width:1},label:{formatter:'1R',color:'#5e626e',fontFamily:'IBM Plex Mono',fontSize:10}}]}
    }]
  },true);
...
function updateHeatmap(){
  ...
  if(type==='prob'){
    ...
    const btype=type==='ext_db'?'double_break':'single_break';
    const label=btype==='double_break'?'Double Break':'Single Break';
