...
for size in sizes:
    ...
    print(f"==================== GENERATING {size}m TESTS ====================")
    for script in scripts_to_convert:
        ...
        # Text modifications
        content = content.replace("60-Minute Window", f"{size}-Minute Window")
        content = content.replace("60-minute Initial Balance", f"{size}-minute Initial Balance")
        content = content.replace("60-Minute IB Window", f"{size}-Minute IB Window")
        content = content.replace("60-minute windows", f"{size}-minute windows")
        content = content.replace("60m rolling", f"{size}m rolling")
    ...
    dash_content = dash_content.replace('IB Research Terminal — NQ & ES', f'IB Research Terminal ({size}m) — NQ & ES')
    dash_content = dash_content.replace('60m rolling · 30m step', f'{size}m rolling · {step}m step')
    ...
    print(f"All {size}-minute tests and dashboard generation completed successfully!\n")
