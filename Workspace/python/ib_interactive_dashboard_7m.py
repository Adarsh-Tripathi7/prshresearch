...
OUT_PATH = r"d:\Antigravity\Dashboard\ib_interactive_dashboard_7m.html"
...
# Load all datasets
prob = read_csv_json("db_probability_by_rth_close_7m.csv")
prob_first = read_csv_json("db_probability_by_first_break_7m.csv")
corr = read_csv_json("ib_correlation_directional_7m.csv")
...
for asset in ["nq", "es"]:
    ...
    for btype in ["double_break", "single_break", "all_breaks"]:
        ...
        for direction in ["combined", "high_first", "low_first"]:
            fname = f"{asset}_{btype}_{direction}_percentiles_7m.csv"
...
<title>IB Research Terminal (7m) — NQ & ES</title>
...
(function(){
  ...
  $('kpiStrip').innerHTML=`
    <div class="metric"><div class="metric-label">Windows Analyzed</div><div class="metric-value" style="color:var(--text-1)">172</div><div class="metric-sub">7m rolling · 7m step</div></div>
