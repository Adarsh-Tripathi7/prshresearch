...
def build_sessions(df):
    ...
    sess_starts = np.zeros(num_weeks, dtype=np.int32)
    sess_ends   = np.zeros(num_weeks, dtype=np.int32)
    
    curr = week_idx_arr[0]; sess_starts[curr] = 0
    for i in range(1, len(week_idx_arr)):
        if week_idx_arr[i] != curr:
            sess_ends[curr] = i - 1
            curr = week_idx_arr[i]
            sess_starts[curr] = i
    sess_ends[curr] = len(week_idx_arr) - 1
